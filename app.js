...
async function openCamera() {
  if (!navigator.mediaDevices?.getUserMedia) {
    setStatus('Camera non disponible sur ce navigateur.');
    return;
  }

  openCameraBtn.disabled = true;
  openCameraBtn.textContent = 'Ouverture...';
  ...
  try {
    ...
    setStatus('Camera ouverte. Prends la photo quand le cadrage est bon.');
  } catch (error) {
    stopCameraStream();
    cameraModal.classList.add('hidden');
    setStatus('Impossible d\'ouvrir la camera. Verifie l\'autorisation du navigateur.');
  } finally {
    openCameraBtn.disabled = false;
    openCameraBtn.textContent = 'Ouvrir la camera';
  }
}
...
function capturePhoto() {
  if (!cameraStream || !cameraVideo.videoWidth || !cameraVideo.videoHeight) {
    setStatus('La camera n\'est pas encore prete.');
    return;
  }
  ...
  loadImageFromDataUrl(cameraCanvas.toDataURL('image/jpeg', 0.92));
  imageInput.value = '';
  closeCamera();
  setStatus('Photo capturee. Place d\'abord les 2 points de reference.');
}
